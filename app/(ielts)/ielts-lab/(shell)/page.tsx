...
export default function IeltsLabPage() {
  ...
  //
  // All three totals are now static file reads — the Reading catalog, the
  // Speaking corpus and the Writing Task 2 bank. Nothing on this route touches
  // the network, which is why `revalidate` is gone: it existed for the Directus
  // writing sets this page used to load, and there is nothing left to
  // revalidate.
  //
  // Writing passes ids rather than a count so the card's "done" can be the
  // number of *these* questions with a submitted attempt, read from this
  // browser.
  return (
    <LabOverview
      exams={getAllExams()}
