...
>
> ⚠️ **复核以生产实测为准,本地环境与源码阅读仅作辅助**(阶段一复核经验,2026-08-09)。
> 那一轮出过一次阻塞级误判:复核者从源码推断 `middleware.ts` 的 `:path+` 对单段 URL
> 不触发 rewrite,据此判定预览面全面失效;生产实测是 200 + `X-Matched-Path` 指向预览
> 路由,结论被撤回。**判据顺序:线上 curl 输出 > 线上日志 > 本地复现 > 源码推断。**
> 用源码推断出结论时,必须先去线上验一次再写进报告;线上与本地结论冲突时,以线上为准
> 并把差异本身当作发现记录下来。
...
顺序不能乱。⚠️ **2026-08-09 踩过的坑**:本节初稿写的是"改 RAM 策略 → 迁对象 → 切变量",实际执行时第 2 步一保存,子账号就**立刻失去旧桶权限** —— 迁移脚本读不了源端,同时线上环境变量还指着旧桶,下一次构建直接 403 失败(构建期读不到索引就响亮失败是设计行为,见 `lib/oss/catalog.ts`)。**RAM 策略在迁移期间必须同时授权新旧两个桶**。

1. 控制台新建 bucket,地域选 **华南香港 `oss-cn-hongkong`**,其余选项照 7.1 的表(私有、开版本控制)。
2. 改 RAM 自定义策略,**新旧两个桶都列进 Resource**(四条:新旧各一条 `/*` 对象级、各一条桶级)。迁移全程保持双授权。
...
6. 验收:三条 curl 全绿后,把 RAM 策略收回成**只授权新桶**(最小权限),再过几天删旧桶(留回退余地)。

**空库时的捷径**:若库里只有一个空索引(没有任何 published 院校包),第 2–4 步可以整段跳过 —— 新桶什么都不放即可。`readSchoolIndex()` 读不到 `index/schools.json` 会按"空库"处理(NoSuchKey 是被捕获的正常分支),站点渲染空态,构建正常通过。直接建桶、把策略指向新桶、切环境变量就行。
